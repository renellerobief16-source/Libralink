          ...
          {loadingAnnouncements ? (
            ...
            <div className="grid grid-cols-1 gap-3.5">
              {announcements.map((ann) => {
                ...
                const authorPic = ann.author?.profile_picture || ann.creator_profile_picture || ann.profile_image || ann.profile_picture;
                const authorName = ann.author?.name || ann.creator_name || 'Library Administration';
                const authorRole = ann.author?.role || ann.creator_role || (isGlobal ? 'Super Admin' : 'Librarian Admin');
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-3 pb-3 border-b border-slate-100">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="h-10 w-10 shrink-0 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 text-white font-bold text-sm flex items-center justify-center overflow-hidden shadow-2xs border border-blue-100">
                          {authorPic ? (
                            <img
                              ...
                              onError={(e) => {
                                ...
                                const fb = e.currentTarget.parentElement.querySelector('.ann-admin-initials');
                                if (fb) fb.classList.remove('hidden');
                              }}
                            />
                          ) : null}
                          <span className={`ann-admin-initials ${authorPic ? 'hidden' : 'flex items-center justify-center'}`}>
                            {authorInitials}
                          </span>
                        </div>
                  ...
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 border border-blue-100 flex items-center justify-center overflow-hidden shadow-2xs flex-shrink-0 relative text-white font-bold text-sm">
                    {notification.sender_profile_picture ? (
                      <img
                        ...
                        onError={(e) => {
                          e.currentTarget.style.display = 'none';
                          const fb = e.currentTarget.parentElement.querySelector('.notif-lib-initials');
                          if (fb) fb.classList.remove('hidden');
                        }}
                      />
                    ) : null}
                    <span className={`notif-lib-initials ${notification.sender_profile_picture ? 'hidden' : 'flex items-center justify-center'}`}>
